...
# ToolTip_Lang (the AURA description shown when you HOVER A BUFF ICON) — distinct from
# Description (170, spellbook only). Verified vs Power Word: Fortitude (1243) field[187]
# = "Increases Stamina by $s1.". Without this, buff-hover shows the name but no text.
F_TOOLTIP,F_TOOLTIPMASK = 187,203
...
def build_patched_spell_dbc(good):
    ...
    for sid,name,icon,dur,aura,bp,die,stack,desc in AA_BUFFS:
        ...
        # Also write the text into the aura ToolTip field so it shows on BUFF-ICON hover.
        rec[F_TOOLTIP]=add_string(desc); rec[F_TOOLTIPMASK]=LOCALE_MASK
